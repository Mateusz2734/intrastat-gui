...
class SemWindow(BaseWidget):
    def __init__(self):
        super().__init__()
        try:
            self.settings = read_settings()["sem"]
    ...
    def populate_labels(self):
        self.label_choose_file.setText(self.sem_file)
    ...
    def choose_file_handler(self):
        ...
        if fpath[0] != "":
            ...
            self.sem_file = fpath[0]

    def ok_handler(self):
        if (self.db_file and self.sem_file) is not None:
            self.run_worker(desha, self.sem_file, self.db_file)
        else:
